    ...
    : BeerPackageHandler<Add_beer>
{
    public override IEnumerable<object> Handle_command(Add_beer command)
    {
        if (_aggregate.Capacity.Capacity <= _aggregate.Contents.Count)
            yield return new Beer_failed_to_add(command.AggregateId, command.beer, Fail_reason.Box_was_full);
        else
            yield return new Beer_added(command.AggregateId, command.beer);
    }
}